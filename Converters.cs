...
public class ScryfallLegalitiesConverter : JsonConverter<Dictionary<string, Legality>?> {
	public override Dictionary<string, Legality>? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
		...
		while(reader.Read()) {
			...
			if(reader.TokenType != JsonTokenType.PropertyName) { throw new JsonException($"Expect property name at {reader.BytesConsumed + 1} bytes."); }

			string? propertyName = reader.GetString() ?? throw new JsonException($"Property name at {reader.BytesConsumed + 1} bytes is null.");

			reader.Read();
			if(reader.TokenType != JsonTokenType.String) { throw new JsonException($"Value at {reader.BytesConsumed + 1} bytes is not a string."); }
			string value = (string)(reader.GetString() ?? throw new JsonException($"Value at {reader.BytesConsumed + 1} bytes is null."));
			...
			legalities.Add(propertyName, value switch {
				...
				_ => throw new JsonException($"Cannot convert {value} At {reader.BytesConsumed + 1} bytes to {typeof(Legality)}."),
			});
		}
		return legalities;
	}
	...
	public override Dictionary<string, decimal?> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
		...
		while(reader.Read()) {
			...
			if(value == null) {
				...
				throw new JsonException($"Value at {reader.BytesConsumed + 1} is not a number.");
			}
		}
		return prices;
	}
